/*
  ...
  We're initilaize an array of `PlaygroundData` and filter all PlaygroundData's
  with the `id` of `4712`.
*/
...
const reducerFunction: Reducer<Post, ReadonlyArray<Post>> = (
  accumulator,
  currentValue
) => {
...
const filteredResultInline = wholeData.reduce(
  (accumulator: ReadonlyArray<Post>, currentValue: Post) => {
    if (currentValue.id === 4712) {
      return [...accumulator, currentValue];
    }
    return accumulator;
